...
@pytest.fixture(
    params=[
        pytest.param(
            {"ordered_percentage": 0.25, "shuffle_beginning": True},
            id="shuffled_first_25%",
        ),
        pytest.param(
            {"ordered_percentage": 0.25, "shuffle_beginning": False},
            id="shuffled_last_25%",
        ),
        pytest.param(
            {"ordered_percentage": 0.50, "shuffle_beginning": True},
            id="shuffled_first_50%",
        ),
        pytest.param(
            {"ordered_percentage": 0.50, "shuffle_beginning": False},
            id="shuffled_last_50%",
        ),
        pytest.param(
            {"ordered_percentage": 0.75, "shuffle_beginning": True},
            id="shuffled_first_75%",
        ),
        pytest.param(
            {"ordered_percentage": 0.75, "shuffle_beginning": False},
            id="shuffled_last_75%",
        ),
    ]
)
def partially_ordered_list(request: FixtureRequest, ordered_list: list) -> list:
    ...
    if shuffle_beginning:
        ...
        shuffled_remaining_ordered_list: list = random.sample(
            remaining_ordered_list, k=len(remaining_ordered_list)
        )
